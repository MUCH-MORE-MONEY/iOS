//
//  StatisticsDateBottomSheetViewController.swift
//  MMM
//
...
final class StatisticsDateBottomSheetViewController: BottomSheetViewController2, View {
	typealias Reactor = StatisticsDateBottomSheetReactor
	// MARK: - Sub Type
	enum Mode {
		case date
		case onlyMonthly
	...
	func bind(reactor: StatisticsDateBottomSheetReactor) {
		bindState(reactor)
		bindAction(reactor)
	}
}
//MARK: - Bind
extension StatisticsDateBottomSheetViewController {
	// MARK: 데이터 변경 요청 및 버튼 클릭시 요청 로직(View -> Reactor)
	private func bindAction(_ reactor: StatisticsDateBottomSheetReactor) {
		// 확인 버튼
		checkButton.rx.tap
			.withUnretained(self)
	...
	private func bindState(_ reactor: StatisticsDateBottomSheetReactor) {
		reactor.state
			.map { $0.dismiss }
...
extension StatisticsDateBottomSheetViewController: UIPickerViewDataSource, UIPickerViewDelegate {
