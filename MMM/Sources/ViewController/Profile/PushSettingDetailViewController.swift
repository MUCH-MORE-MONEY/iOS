...
//    private let  weekList = Observable.of(["일", "화", "수", "목", "금", "토", "월"])
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        if let num = Common.getCustomPushTime().stringToInt() {
            Tracking.NotiSetting.timeLogEvent(num)
        }
...
extension PushSettingDetailViewController {
    private func presentBottomSheet(_ isPresent: Bool) {
        ...
        let date = Common.getCustomPushDate()
        
        let vc = DateBottomSheetViewController(title: "시간 설정", type: .time, date: date, height: 360, mode: .date, sheetMode: .drag, isDark: false)
